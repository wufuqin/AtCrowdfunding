...
<%--查询数据--%>
<script>
    function queryPageCert(pageIndex) {
        $.ajax({
            type : "POST",
            data : {
                "pageno" : pageIndex + 1,
...
<%--模糊查询--%>
<script>
    function queryPageCertLike(pageIndex) {
        $.ajax({
            type : "POST",
            data : {
                "pageno" : pageIndex + 1,
            ...
            success : function (result) {
                ...
                if (result.success){
                    ...
                    /*判断返回的集合中是否有数据*/
                    if (data.length == 0){
                        layer.msg("没有您要查询的资质信息",{time:2000, icon:6, shift:6});
                        return false;
                    }
                    var content = '';
                    /* 对后台返回的数据进行拼串展示 */
                    ...
                    $("#Pagination").pagination(page.totalsize, {
                        num_edge_entries: 2, //边缘页数
                        num_display_entries: 4, //主体页数
                        callback: queryPageCertLike, //当前函数
                        items_per_page:8, //每页显示多少条
                        current_page :(page.pageno-1), //当前页
                        prev_text : "上一页",
                        next_text : "下一页"
